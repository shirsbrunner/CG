 ...
 * 
 */
public class DividedControlLandscapeFlyer
{	
	...
	public static class SimpleMouseListener implements MouseListener, MouseMotionListener
	{
		...
		public void mousePressed(MouseEvent e) {
    		...
    		oldZ = calculateZ(oldX, oldY, zMove);
    		if (oldZ == 0){zMove = true;}
		...
		public void mouseDragged(MouseEvent e) {
    		...
    		newZ = calculateZ(newX, newY, zMove);
			...
			mouseTurn.setIdentity();
			if (!zMove){ //if not a zMove calculate X-Movement
				mouseTurn.set(moveAxisAngle); //the new Multiplication-Matrix
			}
			
			
			//now calculate the around World - z movement (actually world y-movement as it appears this moment)
			...
			if (worldMoveAxisTemp.y < 0){worldMoveAngle = -worldMoveAngle;}
			...
			mouseWorldTurn.setIdentity();
			if (zMove){ //if zMove calculate this
		 ...
		 * @return
		 */
		private float calculateZ(float x, float y, boolean zMove){
			float z = 0;
			if (!zMove){z = 1-x*x - y*y;} //calculate z if not a zMove
