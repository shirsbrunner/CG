...
{		
	...
	for (int i = 0; i < MAXLIGHTS; i++){
		...
		vec4 tempTexture = texture(myTexture, frag_texcoord);
		float nreflectionCoefficient = (tempTexture.x + tempTexture.y + tempTexture.z)/3; 
		
	 	//put it all together
	 	ndotA = lightIntensity *(ndotA);// * nreflectionCoefficient); //use reflectionCoefficient to show working
	 	float ndotAspec = lightIntensity*(nreflectionCoefficient * pow(rDotE,phongExponent));	 	
	 	
		//sum it up
		tempVec += texture(myTexture, frag_texcoord) *(ndotA);
